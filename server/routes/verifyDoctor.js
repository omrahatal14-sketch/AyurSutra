...
//
// Flow:
//   1. Check local MySQL cache → return immediately if found (source: "database")
//   2. If cache miss → run Playwright scraper against MCIM website
//   3. If scraper finds the doctor → INSERT into MySQL cache → return (source: "live_scraped")
...
router.get('/', async (req, res) => {
  ...
  try {
    // ── Step 1: Check MySQL cache first ─────────────────────────────────
    ...
    if (rows.length > 0) {
      console.log(`[Verify] Cache HIT for regNo: ${registrationNumber}`);
      return res.json({
        success: true,
        source: 'database',
        data: {
    ...
    // ── Step 3: INSERT scraped data into MySQL cache for future lookups ──
    await pool.query(
    ...
    console.log(`[Verify] Scraped and cached regNo: ${registrationNumber}`);
